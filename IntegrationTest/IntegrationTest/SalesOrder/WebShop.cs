...
namespace IntegrationTest.SalesOrder
{
    [AllureNUnit]
    public class WebShop
    {
        private async Task WebshopOrder(string orderType, string apiType = "Standard")
        {
            ...
            string orderHubUrl = $"http://{domain}/WebApiClient/RemoteService/OrderHubOrdersJson?orderType={orderType}&apiType={apiType}";
            ...
            Console.WriteLine($"orderHubContent: {orderHubContent}");
        ...
        // need change when to run this test, once per week only
        
        [Test]
        [AllureOwner("Samy Kacem")]
        [AllureIssue("Bug : OCTP-7898", "https://galenica.atlassian.net/browse/OCTP-7898")]
        [AllureTms("Test case : n/a")]
        [AllureLink("Story/Epic : n/a")]
        [AllureDescription("This test check if a Web Online Order is arriving in APOS")]
        public async Task WebshopOrder_NoPrescription()
        {
            // Check if today is Friday and run the test
            ...
            {
                Assert.Ignore("Test is configured to run only on Friday.");
            }
        }
        
        [Test]
        [AllureOwner("Samy Kacem")]
        [AllureIssue("Bug : OCTP-7899", "https://galenica.atlassian.net/browse/OCTP-7899")] 
        [AllureTms("Test case : n/a")]
        [AllureLink("Story/Epic : OC-1402", "https://galenica.atlassian.net/browse/OC-1402")]
        [AllureDescription("This test check if a Web Online Repetition Prescription is arriving in APOS")]
        public async Task WebshopOrder_WithRepetition()
        {
            string targetVersion = "25.1";
            string domain = Helpers.SutName.GetDomain();
            bool isGreaterOrEqualVersion = await Helpers.TestFilter.IsGreaterOrEqualVersion(domain, targetVersion);
            Console.WriteLine($"Curent brand name: {SutName.GetBrandFromDomain().ToUpper()}");
            ...
            else if (SutName.GetBrandFromDomain().ToUpper() != "SUN")
            {
                Assert.Ignore("Test is configured to run only on SUN007 and SUN008 and local dev like CGAL60280.");
            }
            else
                await WebshopOrder("WithRepetition", "Rx");
        }
